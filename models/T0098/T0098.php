<?php


///**************************************************************************
//                Intranet - DAVÓ SUPERMERCADOS
// * Criado em: 16/05/2012 por Rodrigo Alfieri
// * Descrição: Relatório Vendas Assistidas
// * Entrada:   
// * Origens:   
//           
//**************************************************************************
//*/
//
//
class models_T0098 extends models
{

    public function __construct($conn,$verificaConexao,$db)
    {
        parent::__construct($conn,$verificaConexao,$db);
    }
    
    public function inserir($tabela,$campos)
    {
        
        $insere = $this->exec($this->insere($tabela, $campos));
        
//       if($insere)
//            $this->alerts('false', 'Alerta!', 'Incluido com Sucesso!');
//       else
//            $this->alerts('true', 'Erro!', 'Não foi possível Incluir!');
       
       return $insere;
    }   
    
    public function altera($tabela,$campos,$delim)
    {              
       $altera = $this->exec($this->atualiza($tabela, $campos, $delim));
       
//       if($altera)
//            $this->alerts('false', 'Alerta!', 'Alterado com Sucesso!');
//       else
//            $this->alerts('true', 'Erro!', 'Não foi possível Alterar!');          
       
       return $altera;
    }   
    
    public function excluir($tabela, $delim)
    {
        $exclui = $this->exec($this->exclui($tabela, $delim));
    }    
    
    public function retornaDados($loja, $dataI, $dataF, $tipo, $filtro)
    {   $connORA  =   $this->consulta;
        
        $sql = "SELECT A.SGE_LOJA         LOJA
            , to_char(A.SGE_DAT_EMISSAO, 'dd/mm/yyyy')    DATA
            , A.SGE_NUM_CUPOM             CUPOM
            , A.SGE_CERTIFICADO           CERTIFICADO
            , A.SGE_CPF                   CPF
            , R.CLV_RAZAO_SOCIAL          CLIENTE
            , A.SGE_PRECO_GAR             VALOR
            , nvl(T.TIP_RAZAO_SOCIAL,'NAO CADASTRADO RMS') NOME
            , DECODE( T.TIP_NATUREZA,'OC'
            , 'OPERADOR DE CAIXA'
            , 'VENDEDOR')                                  OPER_VEND
            , DECODE(A.SGE_DAT_CANCELAMENTO, NULL, 'ATIVO', 'CANCELADO') STATUS
             FROM RMS.SEG_GARITEM A
             LEFT JOIN RMS.AA1CCVAR R ON    (R.CLV_CGC_CPF = A.SGE_CPF)
             LEFT JOIN RROCHA.DATA_RMS H ON (H.DRDATE = trunc(A.SGE_DAT_EMISSAO))
             LEFT JOIN RMS.AA2CTIPO T  ON (A.SGE_VENDEDOR = T.TIP_CODIGO)
             WHERE A.SGE_NUM_CUPOM              > 0";        

        if (!empty($loja))
            $sql    .=  " AND A.SGE_LOJA                 =   $loja";
        if (!empty($filtro))
            $sql    .=  " AND A.SGE_CPF                  =   $filtro";
        if (!empty($tipo))
            $sql    .=  " AND DECODE(A.SGE_DAT_CANCELAMENTO, NULL, 'ATIVO', 'CANCELADO')  =   '$tipo'";
        if (!empty($dataI))                    
            $sql    .=  " AND H.DRDATER7                 >= '$dataI'";
        if (!empty($dataF))
            $sql    .=  " AND H.DRDATER7                 <= '$dataF'";
        
        $sql    .=      " ORDER BY 1,2,6 DESC";
        
        $stid    = oci_parse($connORA, $sql);
        oci_execute($stid);
        return($stid);
    }
       
    public function retornaPerfil($user)
    {
        $sql    =   "  SELECT T0409.T009_codigo  CodigoPerfil
                         FROM T004_T009 T0409 
                        WHERE T009_codigo IN (47,48) 
                          AND T004_login = '$user'";
        
        return $this->query($sql)->fetchAll(PDO::FETCH_COLUMN);
    }
    
    public function retornaLojas()
    {
        $sql    =   "  SELECT T06.T006_codigo LojaCodigo
                            , T06.T006_nome   LojaNome
                         FROM T006_loja T06
                         JOIN T065_segmento_filiais T65 ON T06.T065_codigo = T65.T065_codigo
                        WHERE T65.T065_codigo  = 1";
        
        return $this->query($sql);
    }   
    
    public function retornaDadosTmp($loja, $dataI,$dataF, $status, $cpf, $conferido)
    {
        //Formata datas
        $dataI  =   $this->formataData($dataI);
        $dataF  =   $this->formataData($dataF);
        
        
        $sql    =   "  SELECT TMP.tmp_loja         LOJA
                            , TMP.tmp_data         DATA
                            , TMP.tmp_cupom        CUPOM
                            , TMP.tmp_codigo       CERTIFICADO
                            , TMP.tmp_cpf          CPF
                            , TMP.tmp_cliente      CLIENTE
                            , TMP.tmp_valor        VALOR
                            , TMP.tmp_status       STATUS
                            , TMP.tmp_nome         NOME
                            , TMP.tmp_tipo         TIPO
                            , T108.T108_conferido  CONFERIDO
                        FROM tmp_conferencia_ge TMP
                       LEFT JOIN T108_conferencia_ge T108 ON T108.T108_codigo = TMP.tmp_codigo
                       WHERE 1  =   1";
        
        if (!empty($loja))
            $sql    .=  " AND TMP.tmp_loja   =  $loja";

        if (!empty($dataI))
            $sql    .=  " AND TMP.tmp_data >= '$dataI'";

        if (!empty($dataF))
            $sql    .=  " AND TMP.tmp_data <= '$dataF'";

        if (!empty($status))
            $sql    .=  " AND TMP.tmp_status LIKE '%$status%'";

        if (!empty($cpf))
            $sql    .=  " AND TMP.tmp_cpf    LIKE '%$cpf%'";

        if ($conferido==1)
            $sql    .=  " AND T108.T108_conferido = 1";
        else if ($conferido=="0")
            $sql    .=  " AND T108.T108_conferido is null or T108.T108_conferido = 0";
        
        $sql    .=   " ORDER BY 1,2,6 DESC";
        
        return $this->query($sql);
    }
    
    public function retornaT108($codigoCertificado)
    {
        $sql    =   "SELECT * 
                       FROM T108_conferencia_ge T108
                      WHERE T108.T108_codigo    =   $codigoCertificado";
        
        $dados  =   $this->query($sql);
        $i  =   0;
        foreach($dados  as $campos => $valores)
        {
            $i++;
        }
        
        return $i;
    }
    
}
 ?>
