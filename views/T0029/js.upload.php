<?php
//Codigo vem de quando é uma alteração de arquivo
//$cod  = $_POST["T055_codigo"];
...
if (empty($_POST["T055_codigo"]))
   {//valida o tipo, se for vazio, é um upload não existente, se for igual a 1, o usuário esta
    ...
    if (!empty($_FILES["P0029_arquivo"]))
       {      
//        echo "Dados do Arquivo";
//        echo "<br/>";
//        echo "Nome: ".$_FILES["P0029_arquivo"]['name'];
//        echo "<br/>";
//        echo "Tamanho: ".$_FILES["P0029_arquivo"]['size'];
//        echo "<br/>";        
        //DECLARAÇÕES E PARAMETROS
        //Instancia Classe models_T0029
        $objUpload           =  new models_T0029($conn);
        
        //Tamanho total do arquivo
        $tamanho             =  ($_FILES["P0029_arquivo"]['size']/1024)/1024;
        ...
        if ( $tamanho < $parametro_valor )
           {
            ...
            $arquivo             =  $_FILES["P0029_arquivo"]['name'];    
            ...
            //caminho temporário
            $temporario          =  $_FILES["P0029_arquivo"]['tmp_name'];
            //Nome do arquivo
            ...
            if (!empty($CodigoExt))
               {                       
                ...
                $_POST['T004_owner']       = $objUpload->formataLoginAutoComplete($_POST['T004_owner']);
                   ...
                   { //move o arquivo que esta sendo upado no sistema do temporário para o path de destino               
                    if (rename($path_novo, $path_novo_rename))
                       {//renomeia o arquivo recem movido do temporário para o Código do arquivo no banco 
                        ...
                        echo "<script>window.location = '?router=T0029/associar&cod=$codigo_arquivo&nome=".$_POST['T055_nome'].".".$extensao."';</script>";                    
                       }
                    else
           ...
           { //Mostra mensagem caso o arquivo ultrapasse o tamanho limite do parametro de 10MB
            ...
            <script>window.location = '?router=T0029/home';</script>        
            <?php
           }
       }
    else
                    ...
                    if (rename($path_novo, $path_novo_rename))
                       { //renomeia o arquivo recem movido do temporário para o Código do arquivo no banco
                        ...
                        if (unlink($path_bkp))
                           ...
                           $buscaUsuarios =   $objUpload->retornaUsuariosComPermissao($codigo_arquivo);
                           
                           foreach($buscaUsuarios as $campos=>$valores)
                           {

                               $buscaEmail    =   $objUpload->retornaEmail($valores['Login']);
                               ...
                               $buscaArquivo  =   $objUpload->retornaArquivoUnico($codigo_arquivo);
                               ...
                               foreach($buscaArquivo    as $campos=>$valores3)
                               {
                                  ...
                                  $mensagem    =   'Foi disponibilizada uma nova versão de arquivo para o seu usuário.'.PHP_EOL.PHP_EOL;
                                  $mensagem   .=   'Código: '.$codigo_arquivo.PHP_EOL.PHP_EOL;
